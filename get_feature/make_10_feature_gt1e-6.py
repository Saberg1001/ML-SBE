...
DEFAULT_TRAIN_OUTPUT = FEATURE_DIR / "ionic_10_features_random_gt1e-6_train.csv"
DEFAULT_TEST_OUTPUT = FEATURE_DIR / "ionic_10_features_random_gt1e-6_test.csv"
...
def build_chi_range_x_r_avg(row: dict[str, str], path: Path, row_index: int) -> str:
    if row.get("chi_range_x_r_avg", "").strip():
        return row["chi_range_x_r_avg"]
    chi_range = parse_float(row["χₘₐₓ - χₘᵢₙ"], "χₘₐₓ - χₘᵢₙ", path, row_index)
    radius_avg = parse_float(row["rₐₗₗ (pm)"], "rₐₗₗ (pm)", path, row_index)
    return repr(chi_range * radius_avg)
...
def write_selected(input_path: Path, output_path: Path) -> None:
    ...
    with input_path.open("r", encoding="utf-8-sig", newline="") as source:
        ...
        required_columns = {
            *ID_COLUMNS,
            *[feature for feature in SELECTED_FEATURES if feature != "chi_range_x_r_avg"],
            "χₘₐₓ - χₘᵢₙ",
            "rₐₗₗ (pm)",
            TARGET_COLUMN,
        }
        ...
        for row_index, row in enumerate(reader, start=2):
            selected = {column: row[column] for column in output_columns if column != "chi_range_x_r_avg"}
            fill_missing_excl_li_features(selected, input_path, row_index)
            selected["chi_range_x_r_avg"] = build_chi_range_x_r_avg(row, input_path, row_index)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create the 10-feature gt1e-6 random split.")
